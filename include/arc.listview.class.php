...
class ListView
{
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        $optionlist = "<span><select name='sldd' style='width:{$optionlen}px' onchange='location.href=this.options[this.selectedIndex].value;'>\r\n";
        for($mjj=1;$mjj<=$totalpage;$mjj++)
        {
            if($mjj==$this->PageNo)
            {
                $optionlist .= "<span><option value='".str_replace("{page}",$mjj,$tnamerule)."' selected>$mjj</option></span>\r\n";
            }
            else
            {
                $optionlist .= "<span><option value='".str_replace("{page}",$mjj,$tnamerule)."'>$mjj</option></span>\r\n";
            }
        }
        $optionlist .= "</select></span>\r\n";
